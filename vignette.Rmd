...
```{r seeresults, fig.width=10, fig.height=5}
...
The heatmap below shows the sub-cellular distribution of proteins between the two conditions. We find within the 3288 proteins common in all experiments a total of 783 organelle marker proteins representing 11 subcellular niches (Figure 4A, Supplementary Table 7). Using TAGM a additional 1,717 proteins in the unstimulated dataset and 1,713 proteins in the LPS dataset were classified to distinct subcellular regions with high confidence (Figure 4B). The remaining proteins that did not meet the classifier threshold were left unassigned and labelled as “unknown”. As already mentioned above we do not expect all proteins to localise to one main discrete location, many proteins are known to "moonlight" and live in mixed locations. There was however a high degree of correlation between unstimulated and stimulated datasets, with 75% of the identified proteome sharing the same organelle localisation in both conditions.
...
## Translocations
One of the main aims of the THP-1 study in [1] was to use the hyperLOPIT technology to investigate the (spatial) changes in protein localisation between the unstimulated and 12h-LPS stimulated conditions. In this section we look at how the location of proteins differ between conditions by examining their classifications from TAGM.

Re-localisations i.e. proteins that do not localise to the same subcellular niche in both conditions, were extracted and labelled as one of four different translocation events: 
...
3. Type 3: organelle-to-unknown - cases where proteins in the 12h-LPS data are labelled as "unknown" but are localised to one of the 11 organelle classes in the unstimulated data.
4. Type 4: unknown-to-unknown - proteins that have a difference in their joint probability distribution equal to 1 (as calculated by the natural L2 distance between their joint posterior probabilities in each condition, also known as the Euclidean norm).
...
As already mentioned, proteins labelled as "unknown" location, describe proteins that did not meet the classifier threshold to be assigned to one of the 11 discrete organelle categories that were included in our marker set. We not only wish to examine proteins that move between discrete locations (which we call type 1 translocations above) but we also wish to examine cases where in one condition the protein has been given a discrete location, and in another condition it has been described as "unknown", and therefore not belonging to one of the 11 subcellular classes. These cases are still of great interest as although we may not know where the protein has gone, what we do know is it is not localisaed to one the 11 classes. This could imply a number of scenarios including for example, the possibility of the protein having a mixed population and moonlighting between organelles, or it be that the protein localised to a subcellular niche that does not appear in our training data. Whichever reason what we are interested in is not only the change in localtion but the size of the probability change as this implies a change in localisation regardless of organelle residency. 

As an extra measure of stringency we enforce that in cases 2-4 any protein considered as "unknown" must also have a high outlier probability (as per the below code chunk `out = 1e-3`). 

For every protein the natural L2 distance (Euclidean norm) was calculated between the TAGM joint posterior probabilities and translocation events were further filtered and ranked based on this distance to give a succinct subset of translocations for further analysis.
...
We can further examine and plot these translocations using alluvial and circos plots.

```{r circosplot, message=FALSE, warning=FALSE, eval=FALSE}
source("R/circos.R")
...
customChord(unst[tl], lps[tl, ], 
            ...
            labels = TRUE)
```
*Note: labels were optimised in Inkscape and `customChord` was called with `labels = FALSE`*
![*Figure 11. Directional circos plot showing protein translocations (Fig. S4A of [1])*](figures/circos.png){width=50%}

The t-SNE maps below also show the localisation of translocating proteins.
```{r mapTL, fig.width=14, fig.height=7, warning=FALSE, message=FALSE}
source("r/foi.R")
par(mfrow = c(1, 2))

...
BiocManager::install(c("knitr", "VennDiagram", "ggplot2", "dplyr", "kableExtra", "coda", "reshape2"))
...
plot2D(tsne_unst, method = "none", methargs = list(unst), 
       fcol = "markers", main = "Subcellular markers\nunstimulated THP1-cells")
plot2D(tsne_lps, method = "none", methargs = list(lps),  
       fcol = "markers", main = "Subcellular markers12h-LPS stimulated THP1-cells")
...
cmn <- intersect(featureNames(thpLOPIT_lps_mulvey2021), featureNames(thpLOPIT_unstimulated_mulvey2021))
...
par(mfrow = c(1, 2)) 
mycol <- paste0(getStockcol(), 50)
plot2D(tsne_unst, method = "none", methargs = list(unst), 
       fcol = "tagm.mcmc.allocation", col = mycol,
       main = "TAGM-MCMC allocation\n unstimulated THP1-cells")
plot2D(tsne_lps, method = "none", methargs = list(unst), 
       fcol = "tagm.mcmc.allocation", col = mycol,
...
```
<center>**Class-specific protein profiles for proteins classified in the unstimulated THP1-cell experiments**</center>
...
```
<center>**Class-specific protein profiles for proteins classified in the 12h-LPS stimulated THP1-cell experiments**</center>
...
The heatmap below shows the sub-cellular distribution of proteins between the two different conditions.

```{r overlapAssignments}
...
## Translocations
One of the main aims of the THP-1 study in [1] was to use the hyperLOPIT technology to investigate the (spatial) changes in localisation i.e. the re-localisations, of proteins between different the unstimulated and 12h-LPS stimulated conditions. At this stage in the analysis we have two datasets on which to extract this information. In this section we look at how the location of proteins differ between conditions by examining their location as output from the Bayesian classification analysis in the previous section. The table below summaries the locations of the 3288 proteins on which we focus our analysis as they are common across all experiments and replicates.

```{r tableOfLocs}
kable(rbind("Unstimulated" = table(loc_unst), "LPS" = table(loc_lps)))
```

For each protein the predicted localisation was examined in the unstimulated and the LPS-stimulated dataset and re-localisations were mapped and classified into one of four different translocation events: 

1. from one organelle class in the unstimulated condition to a different organelle class in the LPS-stimulated dataset i.e. organelle to organelle, 
2. from an unknown localisation in the unstimulated dataset to a predicted organelle class in the LPS-stimulated dataset i.e. unknown to organelle, 
3. from a predicted localisation in the unstimulated dataset to an unknown location in the LPS-stimulated dataset i.e. organelle to unknown, and finally 
4. a translocation event within the unknown class i.e. a protein that exhibits a large change between posterior probabilities in both conditions and is classified to an unknown location.


In scenarios 2-4 where proteins were labelled as unknown, translocations were only extracted for proteins that strictly were deemed members of the outlier component and also exhibited a high posterior probability (>0.999). For every protein the natural L2 distance was calculated between the TAGM joint posterior probabilities providing an extra source of information which were used to filter the above 4 translocation events.


In the below analysis we first consider 3 scenarios:

(1) organelle to organelles
- Require high posterior probability and low outlier probability for both unstimulated and LPS.
  
(2) unknown to organelle
- Require unstimulated to have a high outlier probability
- Require LPS to have high posterior and low outlier probability

(3) organelle to unknown
- Require unstimulated to have high posterior and low outlier probability
- Require LPS to have a high outlier probability

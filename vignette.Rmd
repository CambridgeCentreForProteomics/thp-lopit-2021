...
```{r figure2_part2, warning=FALSE, message=FALSE, fig.height=8, eval=FALSE}
...
```{r heatmap_12h, fig.width=8, fig.height=10, warning=FALSE, message=FALSE, eval=FALSE}
library("gplots")
library("plotrix")
library("scico")
...
```{r mdiclustering2, message=FALSE, warning=FALSE, fig.height=8, fig.width=10, eval=FALSE}
library("RColorBrewer")
source("r/plotRibbons.R")
...
```
```{r mdisecond, eval=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=3, fig.cap="Bayesian temporal clustering of the LPS time series demonstrates clusters of co-regulated proteins. Shown are 6 representative upregulated (A) and 6 downregulated (B) clusters from the Bayesian temporal clustering analysis"}
par(mfrow = c(3,2), mar = c(6, 5, 3, 3))
for (i in 7:12) {
  ribbonPlot(mdi_profs[[i]], col = ribbonCol[i], c(0.05, .95), 
             main = paste("Cluster", ids[i]), cex.main = 2, size = 2)
...
```{r addGoAnnotationForMDIsi1, fig.width=10, eval=FALSE}
source("r/GObarplot.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/GObarplot.R
...
```{r plotit, eval=FALSE, fig.width=10, fig.height=15, warning=FALSE, message=FALSE, fig.cap="(Top panel) Shown are the t-SNE projections of the unstimulated (0h-LPS) and stimulated (12h-LPS) hyperLOPIT datasets. (Bottom panel) Shown are the same t-SNE projections following TAGM-MCMC classification. 3,288 proteins are shown in each plot and coloured by organelle protein marker classes. Proteins of unknown location are shown in grey."}
source("r/prettymap.R")
...
```{r alluvialplot2, eval=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.cap="An alluvial plot representing the 253 proteins which are found to move between organelles at 12h-LPS. The number of proteins that are found to translocate to and from each subcellular compartment is denoted next to the labelled alluvial blocks. Colours of organelles are matched to the hyperLOPIT organelle colour key. For the Muley et al 2021 manuscipt the figure labels were optimised in Inkscape."}
source("r/riverplot.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/riverplot.R
...
```{r fig5, fig.width=10, fig.height=10, fig.cap="t-SNE maps for the unstimulated (left) and 12h-LPS stimulated (right) showing relocalisation of RHO-GTPase trafficking molecules to the PM", warning=FALSE, message=FALSE}
source("r/prettymap.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
...
```{r fig6, fig.width=10, fig.height=10, fig.cap="t-SNE maps for the unstimulated (left) and 12h-LPS stimulated (right) showing the distribution of 15 proteins which were altered in abundance during the time-course of LPS stimulation and also translocated to different subcellular regions in hyperLOPIT space.", warning=FALSE, message=FALSE}
source("r/prettymap.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
...
```{r fig6profs, eval=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.cap="Median temporal abundance profiles for Sequestosome and CLEC11A proteins."}
source("r/plotLineGraph.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/plotLineGraph.R
...
```{r heatmapfig6, fig.width=8, fig.height=5, fig.cap="A heatmap of the relative quantitative abundance values for these 15 proteins during the time-course experiment.", warning=FALSE, message=FALSE}
library(pRolocdata)
library(pRoloc)
library(gplots)
...
```{r fig7, warning=FALSE, message=FALSE, fig.width=12, fig.height=6, fig.cap="Spatial map of the hyperlOPIT data from 12h-LPS stimulated cells with temporal clusters 9, 11 and 17 overlaid (purple = cluster 9 (chromatin enriched, adj.p-value = 6.9e-10), yellow = cluster 11 (mitochondria enriched, adj.p-value = 9.6e-09), cyan = cluster 17 (cytosol enriched, adj.p-value = 1.8e-13)). A Fisher’s exact test was used to determine that these three clusters were enriched for individual hyperLOPIT organelles."}
source("r/prettymap.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
...
```{r fig7ribbons, warning=FALSE, fig.width=12, fig.height=4, fig.cap="The temporal profiles of the three Bayesian temporal clusters during the 24h-LPS time-course"}
library("RColorBrewer")
source("r/plotRibbons.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/plotRibbons.R
...
```{r fig7c, message=FALSE, warning=FALSE, fig.width=10, fig.height=10, fig.cap="Several protein complexes were overlaid onto the hyperLOPIT plot.Protein-protein interaction partner pairs from the literature were shown to co-localise with each other in hyperLOPIT space"}
source("r/foi.R")
source("r/prettymap.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
...
```{r SIfig1, warning=FALSE, fig.width=10, fig.height=5, fig.cap="t-SNE spatial map showing the localisation of four cytokine proteins that were imputed from incomplete TMT profiles and overlaid onto the LPS stimulated hyperLOPIT map, to give an indication of their steady state localiation."}
source("r/prettymap.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
...
```{r figs1g, fig.width=12, eval=FALSE, fig.height=10, warning=FALSE, message=FALSE, fig.cap="Heatmap showing the distribution and overlap in organelle assignments between the TAGM classifications in the unstimulated data and classifications in the LPS-stimulated data"}
source("R/intersection.R")
## Add a label to the LPS dataset so when we combine it  
## with the unstimulated the information is not lost
.lps <- lps
...
```{r SIfig2, fig.width=10, fig.height=10, fig.cap="t-SNE map showing the 253 translocating proteins from the hyperLOPIT experiments and plotted by translocation class."}
source("r/prettymap.R")
source("r/foi.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/foi.R
...
```{r circosplot2, eval=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=9, fig.height=9, fig.cap="Circos plot showing the flow of translocating proteins between different subcellular compartments."}
source("R/circos.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/circos.R
...
```{r figSIBCD, fig.width=10, fig.height=15, eval = FALSE, fig.cap="t-SNE maps showing (top) 67 proteins which were found to translocate to or from the cytosol cluster. (middle) Shown are 57 proteins which were found to translocate to or from the nucleus cluster. (bottom) Shown are 49 proteins which were found to translocate to or from the lysosome cluster."}
source("r/prettymap.R")
source("r/foi.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/foi.R
...
```{r figSI3, fig.width=10, fig.height=18, fig.cap="Summary of nucleo-cytoplasmic and plasma membrane translocation events. (A) Shown are 37 proteins which were found to translocate between the cytosol and the nuclear clusters. (B) Shown are 9 proteins found to translocate between subnuclear compartments. (C) Shown are 70 proteins found to translocate to or from the PM. (D) Shown are three proteins known to co-localise: SQSTM1, CALCOCO2, TOM1", message=FALSE, warning=FALSE}
source("r/prettymap.R")
source("r/foi.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/foi.R
...
```{r figSI4, fig.width=10, fig.height=15, fig.cap="T-SNE plots of translocating proteins involved in trafficking. (A) Shown are 8 trafficking proteins which were found to translocate following LPS stimulation. (B) Shown are 12 RAB GTPase trafficking proteins which were found to translocate following LPS stimulation. (C) 11 members of the ESCRT complex."}
source("r/prettymap.R")
source("r/foi.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/foi.R
...
```{r figSI5, fig.width=12, fig.height=21, fig.cap=" The hyperLOPIT plots can be used as a scaffold to overlay proteins of interest, such as those identified by other subcellular proteomic studies. Shown are organellar proteins identified in four different studies, which have been overlaid onto the hyperLOPIT plots to validate their subcellular localisations. (A) Secretome proteome (Meissner et al., 2013), (B) cell surface proteome (Kalxdorf et al., 2017), (C) the RNA binding proteome (Liepelt et al., 2016) and (D) mitochondrial and lysosomal proteome (Fu et al., 2020)."}
source("r/prettymap.R")
source("r/foi.R")
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/prettymap.R
# https://github.com/CambridgeCentreForProteomics/thp-lopit-2021/blob/main/R/foi.R
